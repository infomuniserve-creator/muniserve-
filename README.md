...
- [x] Staff dashboard redesign + Business Registry — see CLAUDE.md section 7e. Card-based, soft-rounded design system (Quicksand/Nunito, brand navy→teal, light+dark) applied across all four role dashboards, shared `ui.tsx` component library. New Business Registry page (`/dashboard/businesses`, all roles, read-only) lists every business on file with a derived status (active/needs renewal/legacy/inactive/in progress) and search/filter; BPLO can start a renewal or reactivation for a walk-in owner directly from a row. Migration `0010_bplo_walkin_application_insert_policy.sql` is live in production. Verified: clean production build, migrations applied and confirmed live, unauthenticated redirect/login/apply pages smoke-tested in the browser preview with no console errors. **Not yet verified:** the authenticated dashboard views themselves — that needs a real Google sign-in per role, same as the earlier end-to-end testing pattern; automation can't drive Google OAuth.
