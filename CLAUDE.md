...
**Surfaced in the same three places as the plain link** (7o's earlier follow-up) rather than a fourth new location: a new shared `EmbedCodeBox` component (`src/components/embed-code-box.tsx`, textarea + copy button, self-contained inline styles so it works identically on both the Tailwind-styled `/dashboard/staff` and the plain-styled `/admin`) sits right next to the plain apply link on both pages — collapsed behind a native `<details>` disclosure on `/admin` specifically, since that page lists every client and an always-expanded snippet per row would make the list unwieldy; always visible on `/dashboard/staff`, since a BPLO only ever sees their own one client's link.
