...
- **Cooperatives (RA 6938) are exempt from the Mayor's Permit fee entirely** for their manufacturing/exporting activities (Section 4.02(b)) — not just LBT, which was already known. This is an exemption condition, not a fee, so it doesn't belong in `fee_rules` as data — it needs to be a check the fee computation engine (build order step 6) makes before applying Section 4 logic at all, and the application form needs a way to capture "is this a registered RA 6938 cooperative doing manufacturing/exporting?" Not yet designed.
